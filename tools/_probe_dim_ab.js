/* A/B: near-black SIDEDIM. If the mapping is right the sides go black and
 * tops stay lit. SUITE: exclude */
...
D3X.SIDEDIM='#120a05';
let cleared=0;
D3X.dice.forEach(d=>{d.obj.traverse(o=>{
 if(o.isMesh&&o.material&&o.material.userData&&o.material.userData.liveMap){
  const lm=o.material.userData.liveMap;
  if(lm.userData&&lm.userData.dimMaps){delete lm.userData.dimMaps;cleared++;}
 }});});
await sleep(900);
return {cleared,vals:D3X.dice.filter(d=>d.match&&d.phys).map(d=>d.phys.v)};
