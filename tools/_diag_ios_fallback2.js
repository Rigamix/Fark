/* Unforced control THEN forced fallback, same match, same dice - and the
 * painter instrumented for call count + swallowed exceptions. SUITE: exclude */
...
/* instrument the painter - the caller's catch(e3) swallows throws */
...
_diceShadowPaint = function () { window._pCalls++;
  ...
  catch (e) { window._pErr = String(e && e.stack || e); throw e; } };
...
const out = { dieCount: G.pool ? G.pool.length : -1,
...
_dsDirty(); await sleep(700);
_drawDiceShadows(0.8);
