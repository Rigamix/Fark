/* Why does nothing reach phys.v? Dump the dice state after a roll. SUITE: exclude */
...
if(!ok)return {err:'no idle',errs:window.__errs};
handleRoll();
await sleep(6000);
return {pool:G.pool?G.pool.length:-1,phase:G.phase,
 tbl:!!(window.D3X&&D3X._tbl),
 dice:(window.D3X?D3X.dice:[]).filter(d=>d.match).slice(0,6).map(d=>({
  roll:!!d.roll,phys:!!d.phys,v:d.phys?d.phys.v:undefined,t:d.phys?!!d.phys.t:undefined,
  vis:!!(d.obj&&d.obj.visible)})),
