...
# THE BLANK FACES. Denis removed the pips on purpose: they are drawn in code
# now (D3X._pipSheet) so shape, colour and effects stay flexible, and so each
# die can randomise which wear-variant sits on each face - which a baked atlas
# cannot express. These six are VARIANTS, not numbers.
...
for slot, v in enumerate(sorted(FACE)):
    src = os.path.join(ART, '%d.png' % v)
    if not os.path.exists(src):
        sys.exit('GATE FAILED: missing face art ' + src)
    # DROP the alpha; do NOT composite onto white. These carry a vestigial
    # near-opaque alpha (242-255) and blending lightens four of the six by up
    # to 12/255 - visible on hand-painted stone.
    im = Image.open(src).convert('RGB').convert('RGBA')
...
# AND THE WRAPPER, FROM THE SAME BYTES IN THE SAME RUN. die_glb.js is what the
# game loads (window.FK_DIE_GLB); die_cube.glb is only the loader's fallback.
# Regenerating one and not the other left the game drawing the OLD PIPPED
# atlas - invisible, because the code pips sit on the measured positions of
# the painted ones and overlapped them exactly. Only file identity caught it.
import base64 as _b64
io.open(os.path.join(OUTDIR, 'die_glb.js'), 'w', encoding='utf-8',
        newline='\n').write(
    '/* GENERATED by tools/make_cube_glb.py - do not hand-edit.\n   die_cube.glb as base64 (file:// safe). Hard-edged cube: 24 verts, 12 tris,\n   a 3x2 atlas of the six BLANK wear variants. This sheet carries NO pips -\n   D3X._pipSheet composites them at runtime. */\nwindow.FK_DIE_GLB="' + _b64.b64encode(glb).decode() + '";\n')
