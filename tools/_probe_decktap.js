/* Deck-slot tap after a claim -> read-only focus above the slot. SUITE: exclude */
...
await sleep(3000);
tap([...document.querySelectorAll('#end-ov .fo-offer .fcv')][0]);await sleep(600);
tap(document.querySelector('#cardFocusTip .cft-btn'));await sleep(1200);
const slot=document.querySelector('#end-ov .fo-slot.filled .fcv');
tap(slot);await sleep(800);
const t=document.getElementById('cardFocusTip');
const r=t?t.getBoundingClientRect():null,sr=slot?slot.getBoundingClientRect():null;
return {slotTapped:!!slot,tip:!!t,tipVisible:vis(t),
 noBtn:!(t&&t.querySelector('.cft-btn')),
 tipAboveSlot:!!(r&&sr&&r.bottom<=sr.top+2),
